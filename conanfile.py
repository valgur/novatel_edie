...
class NovatelEdieConan(ConanFile):
    ...
    def validate(self):
        ...
        if (self.options.shared or self.options.build_dynamic_libs) and not self.dependencies["spdlog"].options.shared:
            raise Exception("spdlog must be dynamically linked when building novatel_edie as a shared library")
    ...
    def package_info(self):
        ...
        self.runenv_info.define_path("EDIE_DATABASE_FILE",
                                     os.path.join(self.package_folder, "res", "messages_public.json"))
