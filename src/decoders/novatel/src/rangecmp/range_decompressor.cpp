                                                               ...
                                                               ChannelTrackingStatusStruct::SIGNAL_TYPE eSignal_, uint16_t usPRN_)
{
    ...
    else if (stLocktimeInfo
                 .bLocktimeSaturated) // If the locktime marked as saturated, but is not reported as such from the RANGECMP2 message, clear the flag.
    {
        ...
        if (ucLocktimeBits_ == 0 || (stLocktimeInfo.ucLocktimeBits != 0xFF && ucLocktimeBits_ > stLocktimeInfo.ucLocktimeBits))
        {
            ...
            if (dLocktimeChangeMilliseconds != 0)
            {
                pclMyLogger->warn("Detected a locktime jump of {}ms at time {}w, {}ms. SYSTEM: {}, SIGNAL: {}, PRN: {}.",
                                  abs(dLocktimeChangeMilliseconds), stMetaData_.usWeek, stMetaData_.dMilliseconds, static_cast<int32_t>(eSystem_),
                                  static_cast<int32_t>(eSignal_), uiPRN_);
            }
        }
