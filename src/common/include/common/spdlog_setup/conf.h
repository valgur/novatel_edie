 ...
 * @throw setup_error
 */
template <class... Ps> void from_file_with_tag_replacement(const std::string& pre_toml_path, Ps&&... ps);
 ...
 */
template <class... Ps>
auto from_file_and_override_with_tag_replacement(const std::string& base_pre_toml_path, const std::string& override_pre_toml_path,
                                                 const Ps&... ps) -> bool;
 ...
 * @throw setup_error
 */
auto from_file_and_override(const std::string& base_toml_path, const std::string& override_toml_path) -> bool;
 ...
 * @throw setup_error
 */
void save_logger_to_file(const std::shared_ptr<spdlog::logger>& logger, const std::string& toml_path, const bool overwrite = false);
 ...
 * @throw setup_error
 */
auto delete_logger_in_file(const std::string& logger_name, const std::string& toml_path) -> bool;
...
template <class... Ps> void from_file_with_tag_replacement(const std::string& pre_toml_path, Ps&&... ps)
{
    ...
    try
    {
        ...
        details::read_template_file_into_stringstream(toml_ss, pre_toml_path, forward<Ps>(ps)...);
                                                 ...
                                                 const Ps&... ps) -> bool
{
    ...
    try
    {
        ...
        details::read_template_file_into_stringstream(base_toml_ss, base_pre_toml_path, ps...);
        ...
        if (has_override)
        {
            ...
            details::read_template_file_into_stringstream(override_toml_ss, override_pre_toml_path, ps...);
...
inline auto from_file_and_override(const std::string& base_toml_path, const std::string& override_toml_path) -> bool
{
...
inline void save_logger_to_file(const std::shared_ptr<spdlog::logger>& logger, const std::string& toml_path, const bool overwrite)
{
    ...
    try
    {
        const auto config = ([overwrite, &toml_path] {
            if (overwrite) { return cpptoml::make_table(); }
            else { return cpptoml::parse_file(toml_path); }
        })();

        if (!config) { throw setup_error(format("Unable to parse file at '{}' for saving", toml_path)); }
        ...
        const auto curr_loggers = ([&config_ref] {
            ...
            if (curr_loggers) { return curr_loggers; }
            else
            {
        ...
        const auto found_curr_logger = find_item_by_name(curr_loggers_ref, logger->name());

        const auto curr_logger = ([&found_curr_logger, &curr_loggers_ref, &logger] {
            if (found_curr_logger) { return found_curr_logger; }
            else
            {
                const auto new_curr_logger = cpptoml::make_table();
                new_curr_logger->insert(NAME, logger->name());

                curr_loggers_ref.insert(curr_loggers_ref.end(), new_curr_logger);
...
inline auto delete_logger_in_file(const std::string& logger_name, const std::string& toml_path) -> bool
{
    ...
    try
    {
        ...
        if (!config) { throw setup_error(format("Unable to parse file at '{}' for deleting logger '{}'", toml_path, logger_name)); }

        const auto& config_ref = *config;
        const auto curr_loggers = config_ref.get_table_array(LOGGER_TABLE);

        if (!curr_loggers) { throw setup_error(format("Unable to find any logger table array for file at '{}'", toml_path)); }

        auto& curr_loggers_ref = *curr_loggers;

        const auto found_curr_logger_it = find_item_iter_by_name(curr_loggers_ref, logger_name);

        if (found_curr_logger_it == curr_loggers_ref.end()) { return false; }
