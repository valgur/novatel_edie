...
void init_novatel_range_decompressor(nb::module_& m)
{
   nb::class_<oem::RangeDecompressor>(m, "RangeDecompressor")
      ...
      .def("decompress", [](oem::RangeDecompressor& self, nb::bytes data, oem::MetaDataStruct& metadata,
         ...
         if (data.size() > MESSAGE_SIZE_MAX-1)
      ...
      // For unit tests
      .def_static("_reencode_ChannelTrackingStatusWord", [](uint32_t uiCTS) {
         return oem::ChannelTrackingStatusStruct(uiCTS).GetAsWord();
      })
      .def("_set_bitoffset", [](oem::RangeDecompressor& self, uint32_t uiBitOffset_) {
         static_cast<RangeDecompressorTester*>(&self)->SetBitoffset(uiBitOffset_);
      }, "bit_offset"_a)
      .def("_set_bytes_remaining", [](oem::RangeDecompressor& self, uint32_t uiByteCount_) {
         static_cast<RangeDecompressorTester*>(&self)->SetBytesRemaining(uiByteCount_);
      }, "byte_count"_a)
      .def("_get_bytes_remaining", [](oem::RangeDecompressor& self) {
         return static_cast<RangeDecompressorTester*>(&self)->GetBytesRemaining();
      })
      .def("_get_bitfield", [](oem::RangeDecompressor& self, nb::bytes data, uint32_t bitfield_size) {
