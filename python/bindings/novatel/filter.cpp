...
void init_novatel_filter(nb::module_& m)
{
    nb::class_<oem::Filter>(m, "Filter")
        ...
        .def("include_message_id", nb::overload_cast<uint32_t, oem::HEADERFORMAT, MEASUREMENT_SOURCE>(&oem::Filter::IncludeMessageId), "id"_a,
             "format"_a = oem::HEADERFORMAT::ALL, "source"_a = MEASUREMENT_SOURCE::PRIMARY)
        .def("include_message_id",
             nb::overload_cast<std::vector<std::tuple<uint32_t, oem::HEADERFORMAT, MEASUREMENT_SOURCE>>&>(&oem::Filter::IncludeMessageId), "ids"_a)
        .def("invert_message_id_filter", &oem::Filter::InvertMessageIdFilter, "invert"_a)
        .def("include_message_name", nb::overload_cast<std::string, oem::HEADERFORMAT, MEASUREMENT_SOURCE>(&oem::Filter::IncludeMessageName),
             "name"_a, "format"_a = oem::HEADERFORMAT::ALL, "source"_a = MEASUREMENT_SOURCE::PRIMARY)
        .def("include_message_name",
             nb::overload_cast<std::vector<std::tuple<std::string, oem::HEADERFORMAT, MEASUREMENT_SOURCE>>&>(&oem::Filter::IncludeMessageName),
             "names"_a)
