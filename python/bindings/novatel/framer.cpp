...
void init_novatel_framer(nb::module_& m)
{
   nb::class_<oem::Framer>(m, "Framer")
      ...
      .def("set_report_unknown_bytes",
           [](oem::Framer& self, bool report_unknown_bytes) { self.SetReportUnknownBytes(report_unknown_bytes); },
           "report_unknown_bytes"_a)
      .def_prop_ro("bytes_available_in_buffer", [](oem::Framer& framer) { return framer.GetBytesAvailableInBuffer(); })
      .def("get_frame", [](oem::Framer& framer, oem::MetaDataStruct* metadata_ptr, uint32_t buffer_size) {
      ...
      .def("write", [](oem::Framer& framer, nb::bytes data) {
         return framer.Write((unsigned char*) data.c_str(), data.size());
      })
