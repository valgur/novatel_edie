...
void init_novatel_encoder(nb::module_& m)
{
   ...
   nb::class_<oem::Encoder>(m, "Encoder")
      ...
      .def("encode", [](oem::Encoder& encoder, oem::IntermediateHeader& header, PyIntermediateMessage& py_message,
                        oem::MetaDataStruct& metadata, ENCODEFORMAT format)
      {
         oem::MessageDataStruct message_data;
         if (format == ENCODEFORMAT::JSON) {
            ...
            STATUS status = encoder.Encode(
               &buf_ptr, buf_size, header, py_message.message, message_data, metadata, format);
