...
nb::object convert_field(const oem::FieldContainer& field)
{
   if (std::holds_alternative<oem::IntermediateMessage>(field.field_value))
   {
      ...
      else if (message_field[0].field_def->type == field.field_def->type &&
               message_field[0].field_def->name == field.field_def->name)
      {
...
nb::dict& PyIntermediateMessage::values() const
{
   if (cached_values_.size() == 0)
   {
      for (const auto& field: message)
         cached_values_[nb::cast(field.field_def->name)] = convert_field(field);
   }
...
nb::dict PyIntermediateMessage::to_dict() const
{
   ...
   for (const auto& item: values()) {
      if (nb::isinstance<PyIntermediateMessage>(item.second))
      {
         dict[item.first] = nb::cast<PyIntermediateMessage>(item.second).to_dict();
      }
...
size_t PyIntermediateMessage::len() const
{
   return message.size();
}
...
void init_novatel_message_decoder(nb::module_& m)
{
   ...
   nb::class_<oem::MessageDecoder>(m, "MessageDecoder")
      ...
      .def("_decode_ascii", [](oem::MessageDecoder& decoder, const std::vector<BaseField::Ptr>& msg_def_fields,
         ...
         STATUS status = static_cast<DecoderTester *>(&decoder)->TestDecodeAscii(
            msg_def_fields, (char **) &data_ptr, message);
         return nb::make_tuple(status, PyIntermediateMessage(std::move(message)));
      }, "msg_def_fields"_a, "message_body"_a)
      .def("_decode_binary", [](oem::MessageDecoder& decoder, const std::vector<BaseField::Ptr>& msg_def_fields,
