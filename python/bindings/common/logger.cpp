...
void init_common_logger(nb::module_& m)
{
   ...
   nb::class_<spd::logger>(m, "_SpdlogLogger")
      ...
      .def("log", [](spd::logger& logger, spd::level::level_enum level, std::string_view msg) {
         logger.log(level, msg);
      }, "level"_a, "msg"_a)
      .def("trace", [](spd::logger& logger, std::string_view msg) {
         logger.trace(msg);
      }, "msg"_a)
      .def("debug", [](spd::logger& logger, std::string_view msg) {
         logger.debug(msg);
      }, "msg"_a)
      .def("info", [](spd::logger& logger, std::string_view msg) {
         logger.info(msg);
      }, "msg"_a)
      .def("warn", [](spd::logger& logger, std::string_view msg) {
         logger.warn(msg);
      }, "msg"_a)
      .def("error", [](spd::logger& logger, std::string_view msg) {
         logger.error(msg);
      }, "msg"_a)
      .def("critical", [](spd::logger& logger, std::string_view msg) {
         logger.critical(msg);
      }, "msg"_a)
      ...
      .def("__repr__", [](spd::logger& logger) {
   ...
   nb::class_<Logger>(m, "Logging")
      ...
      .def_static("shutdown", &Logger::Shutdown,
                  "Stop any running threads started by spdlog and clean registry loggers")
      .def_static("set_logging_level", &Logger::SetLoggingLevel, "level"_a, "Change the global spdlog logging level")
      .def("register_logger", &Logger::RegisterLogger, "name"_a, "Register a logger with the root logger's sinks.")
      .def_static("add_console_logging", &Logger::AddConsoleLogging,
                  "logger"_a, "level"_a = spd::level::debug,
                  "Add console output to the logger")
      .def_static("add_rotating_file_logger", &Logger::AddRotatingFileLogger,
                  "logger"_a, "level"_a = spd::level::debug, "file_name"_a = "edie.log",
                  "file_size"_a = 5 * 1024 * 1024, "max_files"_a = 2, "rotate_on_open"_a = true,
