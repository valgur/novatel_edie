...
public:
   ...
   static std::u32string Get32ExtensionName(const MultiOutputFileStream& fs) { return cast(fs).s32MyExtentionName; }
...
private:
   static const MultiOutputFileStreamTest& cast(const MultiOutputFileStream& fs) {
      return *static_cast<const MultiOutputFileStreamTest*>(&fs);
   }
};
...
//      .def_prop_ro("file_stream", &FileStream::GetMyFileStream)
   ...
   nb::class_<MultiOutputFileStream>(m, "MultiOutputFileStream")
      ...
      .def("write", [](MultiOutputFileStream& self, nb::bytes& data, std::string msg_name, uint32_t size,
         ...
         return self.WriteData(data.c_str(), data.size(), std::move(msg_name), size, status, week, milliseconds);
      ...
      .def_prop_ro("file_map", [](MultiOutputFileStream& self) {
         nb::dict py_map;
         for (const auto& [key, value] : self.Get32FileMap())
            py_map[nb::cast(key)] = nb::cast(value, nb::rv_policy::reference);
         return py_map;
      })
      .def("set_extension_name", nb::overload_cast<std::u32string>(&MultiOutputFileStream::SetExtensionName), "ext"_a)
      // For unit tests only
