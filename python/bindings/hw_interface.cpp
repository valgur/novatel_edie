...
void init_hw_interface(nb::module_& m)
{
    ...
    nb::class_<MultiOutputFileStream>(m, "MultiOutputFileStream")
        .def(nb::init<>())
        .def("select_file_stream", nb::overload_cast<std::u32string>(&MultiOutputFileStream::SelectFileStream), "file_name"_a)
        .def("clear_file_stream_map", &MultiOutputFileStream::ClearWCFileStreamMap)
        .def("configure_base_file_name", nb::overload_cast<std::u32string>(&MultiOutputFileStream::ConfigureBaseFileName), "file_name"_a)
