...
def get_new_version_file(
    version_file: str, git_server: str, git_token: str, project_id: int, git_hash: str, out_dir: str
):
...
def parse_args():
    ...
    p.add_argument(
        "--git-branch", help="Which branch to submit the new version file to", default="master"
    )
    p.add_argument("--output-path", help="Path to output the new version file")
    p.add_argument(
        "--get_new_version_file",
        help="Calls get_new_version_file()",
        action="store_true",
        default=False,
    )
    p.add_argument(
        "--push_version", help="Calls push_version()", action="store_true", default=False
    )
    args = p.parse_args()
...
def main():
    ...
    if args.get_new_version_file:
        ...
        get_new_version_file(
            version_path, ci_server_url, git_token, git_project_id, git_commit_sha, args.output_path
        )
        print(
            "get_new_version_file arguments:",
            version_path,
            ci_server_url,
            git_project_id,
            git_commit_sha,
    ...
    if args.push_version:
        ...
        with open(version_path) as fp:
            pv_return = release.push_version(
                version_path, ci_server_url, git_token, git_project_id, git_branch, fp.read()
            )
        print(
            "push_version arguments:",
            version_path,
            ci_server_url,
            git_project_id,
            git_branch,
