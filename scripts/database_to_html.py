...
def generate_html_tables_from_json(json_data: dict) -> str:
    ...
    return json2html.convert(
        json=filtered_json_messages, table_attributes='class="table table-bordered table-hover"'
    )
...
def format_html(html: str, enum_data: Enums) -> ET.ElementTree:
    ...
    msg_type_tablist = ET.SubElement(
        msg_type_col,
        "div",
        attrib={"class": "list-group sticky-top", "id": "list-tab", "role": "tablist"},
    )
    ...
    msg_type_content = ET.SubElement(
        msg_type_content_col, "div", attrib={"class": "tab-content", "id": "nav-tabContent"}
    )
...
def generate_msg_type_list_group(
    msg_type: str, msg_type_table: ET.Element, active: bool
) -> ET.Element:
    ...
    content_tablist = ET.SubElement(
        content_tablist_col,
        "div",
        attrib={"class": "list-group", "id": f"{msg_type}-content-tablist", "role": "tablist"},
    )

    content_col = ET.SubElement(content_row, "div", attrib={"class": "col-8 sticky-top h-100"})
    content = ET.SubElement(
        content_col, "div", attrib={"class": "tab-content", "id": f"nav-{msg_type}-tab-content"}
    )
...
def generate_enum_list_group(enum_data: Enums) -> ET.Element:
    ...
    """
    content_tab_pane = ET.Element(
        "div", attrib={"class": f"tab-pane fade", "id": f"list-enums", "role": "tabpanel"}
    )
    ...
    content_tablist = ET.SubElement(
        content_tablist_col,
        "div",
        attrib={"class": "list-group", "id": f"enums-content-tablist", "role": "tablist"},
    )
...
def format_message(msg: ET.Element) -> ET.Element:
    ...
    msg_versions = sorted(
        parsed_msg["fields"].find("table"), key=lambda x: int(x.find("th").text), reverse=True
    )

    accordion = ET.Element(
        "div", attrib={"class": "accordion", "id": f"{parsed_msg['name'].text}-accordion"}
    )
    ...
    for version in msg_versions:
        ...
        accordion_header = ET.SubElement(
            accordion_item, "h2", attrib={"class": "accordion-header", "id": f"{msg_crc}"}
        )
        accordion_button = ET.SubElement(
            accordion_header,
            "button",
            attrib={
        ...
        accordion_collapse = ET.SubElement(
            accordion_item,
            "div",
            attrib={
        ...
        accordion_body = ET.SubElement(
            accordion_collapse,
            "div",
            attrib={"class": "accordion-body overflow-auto", "style": "max-height: 75vh;"},
        )
...
def format_data_type(data_type_col: ET.Element, enum_id_col: ET.Element) -> ET.Element:
    ...
    data_type = {
        data_type_field.find("th").text: data_type_field.find("td").text
        for data_type_field in data_type_fields
    }
    ...
    if data_type["description"]:
        ...
        if data_type["name"] == "ENUM":
            enum_link = ET.SubElement(
                data_type_cell,
                "a",
                attrib={"href": f"#_{enum_id_col.text}", "data-bs-toggle": "modal"},
            )
...
def generate_enum_modals(enum_data: Enums) -> ET.Element:
    ...
    for enum in enum_data:
        ...
        enum_modal_dialog = ET.SubElement(
            enum_modal, "div", attrib={"class": "modal-xl modal-dialog modal-dialog-centered"}
        )
        enum_modal_content = ET.SubElement(
            enum_modal_dialog, "div", attrib={"class": "modal-content"}
        )
        enum_modal_header = ET.SubElement(
            enum_modal_content, "div", attrib={"class": "modal-header"}
        )
        enum_modal_title = ET.SubElement(
            enum_modal_header, "h1", attrib={"class": "modal-title fs-5"}
        )
...
def generate_enum_table(enum_data: Enum) -> ET.Element:
    ...
    for enumerators in enum_data["enumerators"]:
        ...
        for header in enum_table_headers:
            ...
            enum_table_cell.text = (
                str(enumerators[header]) if enumerators[header] is not None else ""
            )
...
def create_tablist(
    parent: ET.Element, label: str, tab_text: str, active: bool = False
) -> ET.Element:
    """
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "-o", "--out_file", help="Output HTML file name. Defaults to the JSON file name"
    )
