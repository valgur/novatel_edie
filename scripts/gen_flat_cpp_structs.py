...
RESERVED_CPP_NAMES = {"switch": "sw", "operator": "opr"}


def gen_flat_structs(
    msg_database: dict, out_file: str = "novatel_message_definitions.hpp", all_logs: bool = False
):
    global CPP_MSG_DEFS
    if all_logs:
...
def gen_field_defs(fields: dict, msg_name: str):
    ...
    for field in fields:
        ...
        if field["type"] == "SIMPLE":
            ...
            msg_fields.append(
                f'{NOVATEL_TO_CTYPES["UCHAR"]} {field["name"]}[{field["arrayLength"]}]'
            )
        elif field["type"] == "FIXED_LENGTH_ARRAY":
            msg_fields.append(
                f'{NOVATEL_TO_CTYPES[field["dataType"]["name"]]} {field["name"]}[{field["arrayLength"]}]'
            )
...
def parse_args():
    ...
    p.add_argument(
        "json_db", help="Path to the NovAtel JSON database", type=lambda x: is_valid_file(p, x)
    )
    p.add_argument(
        "-o",
        "--out_file",
        help='Output header file name. Defaults to "novatel_message_definitions.hpp"',
    ...
    p.add_argument(
        ...
        "--tab_space",
        action="store_true",
        help="Use 4 spaces instead of a tab character",
        default=False,
    )
    p.add_argument(
        "-al",
...
if __name__ == "__main__":
    warnings.warn(
        "This script is will only generate the newest revision of a message in the message definition json."
    )
    print()
